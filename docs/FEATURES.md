...
更新日期：2026-09-20。图片使用当前源码组件与匿名示例数据导出，覆盖全部主要界面与业务明细；顶部标注文档场景，独立悬浮卡不代表实际窗口位置。系统重建确认图使用原生 AppKit 同文案示例，未执行重建。来源与复现方式见[截图说明](images/README.md)。
...
| 状态栏弹窗 | 查看今日 Token、额度、近 7 天额度变化、Token 构成、刷新状态和更新状态 |
...
| 触发入口 | 交互类型 | 展示内容 | 截图 |
| --- | --- | --- | --- |
| 点击菜单栏项目 | 状态栏弹窗 | 套餐、7 天额度、近 7 天变化曲线、今日 Token 构成、刷新、更新、看板、设置与退出 | [截图](images/codexvista-popover.png) |
...
| 悬浮看板趋势节点 | 图表内悬浮卡 | 日期或周期、总 Token、四类 Token、API 等值费用、额度变动、每 1% Token 估算及可展开模型明细 | [折叠](images/codexvista-trend-hover.png) · [模型](images/codexvista-trend-hover-details.png) |
| 悬浮日历日期 | 日历弹窗 | 当日总 Token、四类 Token 构成、API 等值费用、额度变动、每 1% Token 估算及可展开模型明细 | [截图](images/codexvista-calendar-day-hover.png) |
...
- 展示最近 168 小时内的完整额度观测曲线，保留历史周期、消耗和回升；悬停可查看本地观测时间与剩余额度。连续相同额度只保留首尾，无效或冲突观测及跨已过期重置点会断开曲线。
...
- 悬浮趋势节点优先突出当日或当期总 Token，下方展示四类 Token 构成和 API 等值预计花费；按鼠标横向位置选择日期，提示固定锚定该日期最高的可见节点，不随鼠标纵向位置切换曲线；隐藏总量曲线后使用最高的可见模型节点，卡片仍展示该日期或周期的总量。含参考价模型时以 `≈` 标记。
- 趋势与日历悬浮卡默认折叠模型明细；点击“查看模型明细”后展开各模型 Token 构成及等值费用，并保持卡片显示以便滚动阅读。点击“收起模型明细”恢复悬浮预览，点击卡片外或按 Escape 关闭。
- 悬浮卡同时展示本机 7 天额度变动记录，并用同一统计日、重置窗口和套餐内的有效样本估算平均每消耗 1% 额度对应的 Token；样本不足时不显示估算值。额度回升、重置、套餐变化、跨日、无效或过期观测不会跨边界匹配，多设备用量、观测延迟和百分比取整会影响结果。
- 悬浮卡采用固定尺寸和单层滚动；额度记录与模型明细展开后随整张卡滚动，避免原生弹窗在内容变化时反复调整尺寸。
...
- Codex 会话自带仓库信息时，Git 主目录和关联 worktree 按仓库身份合并；缺少仓库信息时按目录身份区分，启动扫描不会访问项目目录。
...
- **任务明细**：任务名称、最后活动时间、回复数、项目内占比和 Token；子 agent 在对应主任务下方缩进为独立行，可单独搜索、排序、悬浮查看模型与 Skills / Tools，并点击进入自身回复明细；项目汇总仍只计主任务聚合值，不因独立展示而重复计数。
...
模型费用说明展示 GPT-6 Astra、Sol、Terra、Luna、GPT-5.5、GPT-5.4、GPT-5.4 Mini 和 Spark 的价格或参考价，并标注核对日期；Spark 使用 GPT-5.5 参考估算。单价保留必要的三位小数，避免将 $0.075 显示为 $0.08。内置价格更新会重新估算历史用量的 API 等值费用，不修改 Token 数据，也不代表历史实际账单。
...
- 长上下文倍率、缓存写入、快速模式和工具调用等无法从当前聚合事件可靠还原的附加费用不计入总额。
...
- Codex 会话元数据中已有的 Git remote，用于识别主目录与 worktree。
